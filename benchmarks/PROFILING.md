...
> **Archived NVX reference:** the command examples below target the original
> NVX CLI. In this repository, build with `scripts/nvx.py build-guest
> --profiling` and use the retained scripts as implementation references.
...
symbol. For exact VMM function names (e.g. `microvm::vm::whp::runtime::execute`,
`microvm::profiler::whp::HostTraceSession::stop`), build the VMM with full debug info:
