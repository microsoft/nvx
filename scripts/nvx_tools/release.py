...
from .build import (
    ...
    validate_shared_status_kernel_config,
    validate_static_x86_64_elf,
    verify_agent_initramfs,
    verify_simple_initramfs,
)
...
RELEASE_TRANSPORTS = (SIMPLE_TRANSPORT, BROKER_TRANSPORT)
...
# ZipFile reads the entire central directory before exposing any members.
RELEASE_ZIP_MAX_DIRECTORY_BYTES = 1024 * 1024
RELEASE_MANIFEST_MAX_BYTES = 1024 * 1024
RELEASE_PACKAGE_MANIFEST_MAX_BYTES = 8 * 1024 * 1024
LIVE_GATE_PROOF_MAX_BYTES = 1024 * 1024
MANIFEST_DIGEST_MAX_BYTES = 128  # One hexadecimal SHA-256 plus text-file whitespace.
# One header and at most one padding block per member, plus final record padding.
...
_GZIP_WINDOW_BITS = 16 + zlib.MAX_WBITS  # zlib's 16 flag selects the gzip wrapper.
_USTAR_NAME_BYTES = 100
_USTAR_PREFIX_BYTES = 155
_ZIP_UNIX_SYSTEM = 3
_ZIP_MODE_SHIFT = 16
_ZIP_ENCRYPTED_FLAG = 0x1
_ZIP_DOS_DIRECTORY_FLAG = 0x10
_ZIP_MIN_TIMESTAMP = (1980, 1, 1, 0, 0, 0)
_ZIP_STREAM_CHUNK_BYTES = 64 * 1024
# PKZIP APPNOTE fixed little-endian structures, including their signatures.
_ZIP_LOCAL_HEADER = struct.Struct("<4s4xH16x2H")  # 30 bytes; method and text lengths.
_ZIP_END_RECORD = struct.Struct("<4s4H2LH")  # 22 bytes.
_ZIP64_LOCATOR = struct.Struct("<4sLQL")  # 20 bytes.
_ZIP64_END_RECORD = struct.Struct("<4sQ2H2L4Q")  # 56 bytes.
_ZIP64_SIZE_PREFIX_BYTES = 12  # Signature and size are excluded from the size field.
_ZIP_DIRECTORY_HEADER = struct.Struct("<4s24x3H12x")  # 46 bytes; three text lengths.
_ZIP_LOCAL_SIGNATURE = b"PK\x03\x04"
_ZIP_END_SIGNATURE = b"PK\x05\x06"
_ZIP64_LOCATOR_SIGNATURE = b"PK\x06\x07"
_ZIP64_END_SIGNATURE = b"PK\x06\x06"
_ZIP_DIRECTORY_SIGNATURE = b"PK\x01\x02"
_ZIP16_MAX = 0xFFFF
_ZIP32_MAX = 0xFFFFFFFF
_ZIP_MAX_COMMENT_BYTES = _ZIP16_MAX
LOCK_TIMEOUT_SECONDS = 60.0
LOCK_POLL_INTERVAL_SECONDS = 0.1
_LOCK_REGION_BYTES = 1  # Windows locks a byte range, even in an empty file.
...
class _BoundedTarStream:
    def __init__(self, path: Path, compressed: bool) -> None:
        ...
        self._decompressor = (
            zlib.decompressobj(_GZIP_WINDOW_BITS) if compressed else None
        )
...
def _canonical_ustar_name_fields(name: str) -> tuple[bytes, bytes]:
    ...
    if len(encoded) <= _USTAR_NAME_BYTES:
        return _tar_text_bytes(name, _USTAR_NAME_BYTES, "name"), bytes(
            _USTAR_PREFIX_BYTES
        )
    ...
    for index in range(1, len(components)):
        ...
        if (
            len(prefix.encode("utf-8")) <= _USTAR_PREFIX_BYTES
            and len(suffix.encode("utf-8")) <= _USTAR_NAME_BYTES
        ):
            return (
                _tar_text_bytes(suffix, _USTAR_NAME_BYTES, "name"),
                _tar_text_bytes(prefix, _USTAR_PREFIX_BYTES, "prefix"),
            )
...
def _preflight_raw_tar(path: Path, *, compressed: bool) -> None:
    ...
    with _BoundedTarStream(path, compressed) as stream:
        while True:
            ...
            # Validate fixed POSIX ustar offsets before tarfile parses extensions.
...
def _validate_root_broker_contract() -> dict[str, object]:
    ...
    _control_contract(manifest, "root")
    return manifest
...
def _preflight_raw_zip(source: BinaryIO) -> int:
    archive_size = source.seek(0, os.SEEK_END)
    if archive_size < _ZIP_END_RECORD.size or archive_size > RELEASE_ARCHIVE_MAX_BYTES:
        raise ScriptError("release ZIP is truncated or exceeds the archive-size limit")
    tail_size = min(archive_size, _ZIP_END_RECORD.size + _ZIP_MAX_COMMENT_BYTES)
    source.seek(archive_size - tail_size)
    tail = source.read(tail_size)
    if len(tail) != tail_size:
        raise ScriptError("release ZIP end record is truncated")
    # Match zipfile's no-comment shortcut before searching for a commented record.
    end_index = len(tail) - _ZIP_END_RECORD.size
    if (
        tail[end_index : end_index + len(_ZIP_END_SIGNATURE)] != _ZIP_END_SIGNATURE
        or tail[-2:] != b"\0\0"
    ):
        end_index = tail.rfind(_ZIP_END_SIGNATURE)
    if end_index < 0 or len(tail) - end_index < _ZIP_END_RECORD.size:
        raise ScriptError("release ZIP end record is missing or truncated")
    (
        _signature,
        disk,
        directory_disk,
        ...
        directory_offset,
        comment_size,
    ) = _ZIP_END_RECORD.unpack_from(tail, end_index)
    if end_index + _ZIP_END_RECORD.size + comment_size != len(tail):
        raise ScriptError("release ZIP end record has an invalid comment length")
    if disk != 0 or directory_disk != 0:
        raise ScriptError("release ZIP must not span multiple disks")
    directory_end = archive_size - tail_size + end_index
    locator_offset = directory_end - _ZIP64_LOCATOR.size
    locator = b""
    if locator_offset >= 0:
        source.seek(locator_offset)
        locator = source.read(_ZIP64_LOCATOR.size)
    if locator.startswith(_ZIP64_LOCATOR_SIGNATURE):
        if len(locator) != _ZIP64_LOCATOR.size:
            raise ScriptError("release ZIP64 locator is truncated")
        _, zip64_disk, record_offset, disks = _ZIP64_LOCATOR.unpack(locator)
        if zip64_disk != 0 or disks != 1:
            raise ScriptError("release ZIP64 must not span multiple disks")
        # Fixed, adjacent ZIP64 records keep Python 3.10+ on the same directory.
        directory_end = locator_offset - _ZIP64_END_RECORD.size
        if directory_end < 0 or record_offset != directory_end:
            raise ScriptError("release ZIP64 locator has an invalid record offset")
        source.seek(record_offset)
        record = source.read(_ZIP64_END_RECORD.size)
        if len(record) != _ZIP64_END_RECORD.size:
            raise ScriptError("release ZIP64 end record is truncated")
        (
            signature,
            record_size,
            _created_version,
        ...
        ) = _ZIP64_END_RECORD.unpack(record)
        if (
            signature != _ZIP64_END_SIGNATURE
            or record_size != _ZIP64_END_RECORD.size - _ZIP64_SIZE_PREFIX_BYTES
        ):
            raise ScriptError("release ZIP64 end record has an unsupported layout")
        for value, zip64_value, sentinel in (
            (disk_members, zip64_disk_members, _ZIP16_MAX),
            (members, zip64_members, _ZIP16_MAX),
            (directory_size, zip64_directory_size, _ZIP32_MAX),
            (directory_offset, zip64_directory_offset, _ZIP32_MAX),
        ):
            if value not in (sentinel, zip64_value):
                raise ScriptError("release ZIP64 central-directory metadata disagrees")
        disk_members = zip64_disk_members
        members = zip64_members
        directory_size = zip64_directory_size
        directory_offset = zip64_directory_offset
    elif (
        disk_members == _ZIP16_MAX
        or members == _ZIP16_MAX
        or directory_size == _ZIP32_MAX
        or directory_offset == _ZIP32_MAX
    ):
        raise ScriptError("release ZIP64 locator is missing")
    if disk != 0 or directory_disk != 0 or disk_members != members:
        raise ScriptError("release ZIP has inconsistent single-disk member counts")
    if members > RELEASE_ARCHIVE_MAX_MEMBERS:
        raise ScriptError("release archive exceeds the member-count limit")
    if directory_size > RELEASE_ZIP_MAX_DIRECTORY_BYTES:
        raise ScriptError("release ZIP exceeds the central-directory size limit")
    if (
        directory_offset + directory_size != directory_end
        or members * _ZIP_DIRECTORY_HEADER.size > directory_size
    ):
        raise ScriptError("release ZIP central-directory bounds are invalid")

    source.seek(directory_offset)
    actual_members = 0
    while source.tell() < directory_end:
        if directory_end - source.tell() < _ZIP_DIRECTORY_HEADER.size:
            raise ScriptError("release ZIP central-directory header is truncated")
        header = source.read(_ZIP_DIRECTORY_HEADER.size)
        if len(header) != _ZIP_DIRECTORY_HEADER.size:
            raise ScriptError("release ZIP central-directory header is truncated")
        signature, name_size, extra_size, comment_size = _ZIP_DIRECTORY_HEADER.unpack(
            header
        )
        if signature != _ZIP_DIRECTORY_SIGNATURE:
            raise ScriptError("release ZIP central-directory header is invalid")
        actual_members += 1
        if actual_members > RELEASE_ARCHIVE_MAX_MEMBERS:
            raise ScriptError("release archive exceeds the member-count limit")
        if actual_members > members:
            raise ScriptError("release ZIP central-directory member count disagrees")
        next_member = source.tell() + name_size + extra_size + comment_size
        if next_member > directory_end:
            raise ScriptError("release ZIP central-directory metadata is truncated")
...
def _preflight_zip_member(
    source: BinaryIO,
    member: zipfile.ZipInfo,
    directory_offset: int,
) -> None:
    description = f"release ZIP entry {member.filename}"
    if not 0 <= member.header_offset <= directory_offset - _ZIP_LOCAL_HEADER.size:
        raise ScriptError(f"{description} has invalid local-header bounds")
    source.seek(member.header_offset)
    header = source.read(_ZIP_LOCAL_HEADER.size)
    if len(header) != _ZIP_LOCAL_HEADER.size:
        raise ScriptError(f"{description} has a truncated local header")
    signature, compression, name_size, extra_size = _ZIP_LOCAL_HEADER.unpack(header)
    if signature != _ZIP_LOCAL_SIGNATURE:
        raise ScriptError(f"{description} has an invalid local header")
    if compression != member.compress_type:
        raise ScriptError(f"{description} has inconsistent ZIP compression methods")
    data_offset = source.tell() + name_size + extra_size
    if (
        member.compress_size < 0
        or data_offset + member.compress_size > directory_offset
    ):
        raise ScriptError(f"{description} has invalid compressed-data bounds")
    if compression == zipfile.ZIP_STORED and member.compress_size != member.file_size:
        raise ScriptError(f"{description} has inconsistent stored sizes")
    ...
    size = 0
    checksum = 0
    # ZipExtFile truncates output to file_size, hiding expansion and a missing EOF.
    while remaining or pending:
        if not pending:
            pending = source.read(min(remaining, _ZIP_STREAM_CHUNK_BYTES))
            if not pending:
                raise ScriptError(f"{description} has truncated compressed data")
        ...
        if decompressor is None:
            ...
            try:
                data = decompressor.decompress(
                    pending,
                    min(_ZIP_STREAM_CHUNK_BYTES, member.file_size - size + 1),
                )
            except zlib.error as error:
                raise ScriptError(
                    f"{description} has an invalid deflate stream: {error}"
                ) from error
            pending = decompressor.unconsumed_tail
            if decompressor.unused_data or (decompressor.eof and remaining):
                raise ScriptError(f"{description} contains trailing compressed data")
        size += len(data)
        if size > member.file_size:
            raise ScriptError(f"{description} exceeds its archive header size")
        checksum = zlib.crc32(data, checksum)
    if decompressor is not None and not decompressor.eof:
        raise ScriptError(f"{description} has a truncated deflate stream")
    if size != member.file_size:
        raise ScriptError(f"{description} is truncated")
    if checksum != member.CRC:
        raise ScriptError(f"{description} has an invalid CRC")


@contextmanager
def _open_release_zip(path: Path) -> Generator[zipfile.ZipFile, None, None]:
    with path.open("rb") as source:
        directory_offset = _preflight_raw_zip(source)
        source.seek(0)
        with zipfile.ZipFile(source) as archive:
            ...
            for member in archive.infolist():
                if member.compress_type not in (
                    ...
                    raise ScriptError(
                        f"release archive entry {member.filename} has an "
                ...
                unix_mode = member.external_attr >> _ZIP_MODE_SHIFT
                ...
                if (
                    member.create_system != _ZIP_UNIX_SYSTEM
                    or stat.S_IFMT(unix_mode) != expected_type
            ...
            if len(roots) != 1:
                raise ScriptError(
                    "release archive must contain exactly one package root"
                )
            if (
                total[0]
                > source.seek(0, os.SEEK_END) * RELEASE_ARCHIVE_MAX_COMPRESSION_RATIO
            ):
                raise ScriptError("release archive exceeds the compression-ratio limit")
            for member in archive.infolist():
                _preflight_zip_member(source, member, directory_offset)
            yield archive
...
def _preflight_release_archive(archive_path: Path) -> _ArchiveInventory:
    ...
    try:
        if archive_path.name.endswith(".tar.gz"):
            ...
            with _open_release_zip(archive_path) as archive:
                for member in archive.infolist():
...
def _sync_file(path: Path) -> None:
    # Windows' file flush requires a writable handle.
    with path.open("r+b" if sys.platform == "win32" else "rb") as contents:
        contents.flush()
...
def _sync_directory(path: Path) -> None:
    try:
        ...
        # The Windows CRT rejects directories, reporting ENOENT for drive roots.
        if (
            sys.platform == "win32"
            and (
...
def _make_directory(path: Path) -> None:
    ...
    # Persist every link, including ancestors left by an interrupted mkdir.
...
def _validate_package_manifest(
    ...
    if (
        ...
        or (transport == SIMPLE_TRANSPORT and not packages)
    ...
    if transport == SIMPLE_TRANSPORT:
        if packaged_agent is not None:
            raise ScriptError("simple package manifest declares a guest agent")
    else:
...
def _validate_release_package_tree(
    ...
    _control_contract(manifest, "packaged")
    ...
    if transport == "broker-ttrpc":
        ...
        if (
            ...
            raise ScriptError("simple package declares an active guest agent")
        verify_simple_initramfs(initramfs)
...
def _read_transaction_journal(transaction: Path) -> dict[str, object]:
    ...
    if (
        ...
        not in ("active", "rolling-back", "committed", "rolled-back")
...
def _rollback_transaction(
    ...
    errors: list[str] = []
    try:
        # Publishing a commit may succeed before the write reports an interruption.
        journal["phase"] = "rolling-back"
        _write_transaction_journal(transaction, journal)
    except BaseException as error:
        return [
            f"{transaction}: cannot persist rollback state: "
        ...
        try:
            if prior_exists:
                if os.path.lexists(backup):
                    ...
                    raise ScriptError(
                        f"cannot restore {destination}: destination and backup are missing"
                    )
            elif os.path.lexists(destination):
                _remove_runtime_path(destination)
            # A previous restore may have renamed the backup before fsync failed.
    ...
    if not errors:
        try:
            ...
            _write_transaction_journal(transaction, journal)
            _remove_runtime_path(transaction)
        except BaseException as error:
            errors.append(
...
def _recover_install_transactions(transaction_parent: Path) -> None:
    ...
    for transaction in sorted(transaction_parent.iterdir()):
        ...
        if journal["phase"] in ("committed", "rolled-back"):
            for operation in _transaction_operations(journal):
                destination = Path(cast(str, operation["destination"]))
                if journal["phase"] == "committed" and operation["kind"] == "replace":
                    _sync_file(destination)
                if destination.parent.exists():
                    _sync_directory(destination.parent)
            # A visible terminal journal may still need its interrupted flush.
            _sync_file(journal_path)
            _sync_directory(transaction)
            _sync_directory(transaction_parent)
            _remove_runtime_path(transaction)
            continue
...
def _interprocess_lock(
    lock_path: Path, description: str
    ...
    with lock_path.open("a+b") as lock:
        ...
        while True:
            try:
                if sys.platform == "win32":
                    msvcrt.locking(lock.fileno(), msvcrt.LK_NBLCK, _LOCK_REGION_BYTES)
                else:
                    fcntl.flock(lock.fileno(), fcntl.LOCK_EX | fcntl.LOCK_NB)
            except OSError as error:
                if error.errno not in (errno.EACCES, errno.EAGAIN):
                    raise ScriptError(
                        f"cannot acquire {description} lock {lock_path}: {error}"
                    ) from error
                remaining = deadline - time.monotonic()
                if remaining <= 0:
                    raise ScriptError(
                        f"timed out waiting for {description} lock {lock_path}; "
        ...
        try:
            ...
            if sys.platform == "win32":
                msvcrt.locking(lock.fileno(), msvcrt.LK_UNLCK, _LOCK_REGION_BYTES)
            else:
                fcntl.flock(lock.fileno(), fcntl.LOCK_UN)


@contextmanager
def _runtime_install_lock() -> Generator[None, None, None]:
    with _interprocess_lock(artifact_path(".install.lock"), "runtime install"):
        yield
...
def _transactional_install(
    ...
    with _runtime_install_lock():
        _transactional_install_locked(files, removals)


def _transactional_install_locked(
    files: list[_InstallFile],
    removals: list[Path],
) -> None:
    transaction_parent = artifact_path(".install-transactions")
    _make_directory(transaction_parent)
...
def create_release_archive(bundle: Path, output: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(
        ...
        if output.name.endswith(".tar.gz"):
            ...
            with zipfile.ZipFile(
                ...
                for path in paths:
                    ...
                    member = zipfile.ZipInfo(arcname, date_time=_ZIP_MIN_TIMESTAMP)
                    member.create_system = _ZIP_UNIX_SYSTEM
                    ...
                    if is_directory:
                        member.external_attr |= _ZIP_DOS_DIRECTORY_FLAG
        ...
        validate_release_archive(temporary)
...
def _control_contract(manifest: object, description: str) -> tuple[int, int, str]:
    ...
    if (abi_version, protocol_version, contract_revision) != (
        MICROVM_ABI_VERSION,
        CONTROL_SESSION_PROTOCOL_VERSION,
        CONTROL_CONTRACT_REVISION,
    ):
        raise ScriptError(f"{description} OpenVMM control contract is unsupported")
...
def _install_release_archive(
    ...
    if expected_transport == SIMPLE_TRANSPORT and trusted_manifest_sha256 is not None:
        raise ScriptError("simple install does not accept broker trust metadata")
    with tempfile.TemporaryDirectory(prefix="nvx-release-") as temporary:
        ...
        packaged_contract = _control_contract(package.manifest, "packaged")
        local_manifest = _json_object(
            _read_limited_file(
                REPO_ROOT / "SOURCE-MANIFEST.json",
                RELEASE_MANIFEST_MAX_BYTES,
        ...
        if package.transport == SIMPLE_TRANSPORT:
...
def download_latest_release(
    ...
    if transport == SIMPLE_TRANSPORT and trusted_manifest_sha256 is not None:
        raise ScriptError(
...
def _publish_release_directory(
    staged: Path,
    destination: Path,
    manifest_digest: tuple[Path, Path] | None = None,
    *,
    force: bool | None = None,
) -> None:
    publications = [(staged, destination)]
    if manifest_digest is not None:
        publications.append(manifest_digest)
    publications = [
        (source, target.parent.resolve() / target.name)
        for source, target in publications
    ]
    fixed_backups = {
        target: target.with_name(f".{target.name}.previous")
        for _, target in publications
    }
    lock_paths = [
        target.with_name(f".{target.name}.publish.lock") for _, target in publications
    ]
    reserved_paths = (
        [target for _, target in publications]
        + list(fixed_backups.values())
        + lock_paths
    )
    if len(set(reserved_paths)) != len(reserved_paths) or any(
        parent in child.parents for parent in reserved_paths for child in reserved_paths
    ):
        raise ScriptError("release publication and backup paths overlap")
    with ExitStack() as locks:
        # Per-path locks also serialize publishers sharing only one destination.
        for lock_path in sorted(lock_paths):
            locks.enter_context(_interprocess_lock(lock_path, "release publication"))
        if force is not None:
            _check_release_destination(publications[0][1], force)
        _publish_release_locked(publications, fixed_backups)


def _publish_release_locked(
    publications: list[tuple[Path, Path]],
    fixed_backups: dict[Path, Path],
) -> None:
    for source, target in publications:
        if target.is_symlink() or (
            target.exists() and target.is_dir() != source.is_dir()
        ):
            raise ScriptError(
                f"release publication has an incompatible target: {target}"
            )
        backup = fixed_backups[target]
        if backup.exists() or backup.is_symlink():
            raise ScriptError(f"stale release backup blocks publication: {backup}")

    backup_roots: dict[Path, Path] = {}
    promotions: set[Path] = set()
    try:
        for _, target in publications:
            backup_roots[target] = Path(
                tempfile.mkdtemp(prefix=f".{target.name}.previous-", dir=target.parent)
            )
        for source, target in publications:
            if target.exists():
                target.replace(backup_roots[target] / "content")
            # A rename may succeed before reporting an interruption.
        ...
        for _, target in reversed(publications):
            try:
                ...
                backup_root = backup_roots.get(target)
                if backup_root is not None:
                    backup = backup_root / "content"
                    if backup.exists():
        ...
        if rollback_errors:
            raise ScriptError(
                f"release publication failed ({error}); rollback also failed; "
                "backups were preserved: " + "; ".join(rollback_errors)
            ) from error
        for backup_root in backup_roots.values():
            _remove_runtime_path(backup_root)
        raise
    for backup_root in backup_roots.values():
        _remove_runtime_path(backup_root)


def _check_release_destination(destination: Path, force: bool) -> None:
...
def _guest_release_inputs(
    ...
    kernel_hashes = _validate_kernel_provenance(
        common["vmlinux"],
    ...
    if transport == SIMPLE_TRANSPORT:
        inputs = {
            **common,
            ...
            "initramfs.cpio.gz.packages.json": require_file(
                artifact_path("initramfs.cpio.gz.packages.json"),
                "simple initramfs package manifest",
            ),
        }
        verify_simple_initramfs(inputs["initramfs.cpio.gz"])
        _validate_package_manifest(
        ...
        return inputs, [inputs["initramfs.cpio.gz.packages.json"]], kernel_hashes
    ...
    return inputs, [agent_packages], kernel_hashes
...
def _validate_kernel_provenance(
    ...
    try:
        ...
        expected_source = json.loads(kernel_source_fingerprint())
    except (json.JSONDecodeError, OSError) as error:
        raise ScriptError(f"invalid kernel build provenance: {error}") from error
    hashes = {
        "vmlinux": sha256_file(kernel),
        "vmlinux.config": sha256_file(kernel_config),
    }
    if (
        provenance.get("format") != 1
        or provenance.get("source_fingerprint") != expected_source
        or provenance.get("kernel_sha256") != hashes["vmlinux"]
        or provenance.get("config_sha256") != hashes["vmlinux.config"]
    ):
        raise ScriptError(
            "kernel build provenance does not match the current source, config, and vmlinux"
        )
    validate_shared_status_kernel_config(kernel_config)
    return hashes
...
def collect_release_sources(transport: str) -> None:
    _guest_inputs, package_manifests, _kernel_hashes = _guest_release_inputs(transport)
    if transport == SIMPLE_TRANSPORT:
...
def package_release(
    *,
    ...
    guest_inputs, package_manifests, kernel_hashes = _guest_release_inputs(transport)
    ...
    with tempfile.TemporaryDirectory(
        ...
        for name, source in guest_inputs.items():
            _copy_release_file(source, staged_destination / "guest" / name)
        for name, expected_sha256 in kernel_hashes.items():
            if sha256_file(staged_destination / "guest" / name) != expected_sha256:
                raise ScriptError(
                    f"packaged {name} does not match its kernel build provenance"
                )
        if transport == BROKER_TRANSPORT:
        ...
        runtime_manifest_path = staged_destination / "SOURCE-MANIFEST.json"
        runtime_manifest_path.write_bytes(
            _runtime_source_manifest(staged_destination, binary.name, transport)
        )
        ...
        runtime_manifest = _json_object(
            runtime_manifest_path.read_bytes(), "staged runtime manifest"
        )
        _verify_runtime_fingerprint(runtime_manifest)
        _normalize_release_tree_modes(staged_destination)
        write_sha256_sums(staged_destination)
        (staged_destination / "SHA256SUMS").chmod(0o644)
        package = _validate_release_package_tree(
            staged_destination,
            transport,
            transport == BROKER_TRANSPORT,
        )
        if digest_output is None:
            _publish_release_directory(
                staged_destination, release_destination, force=force
            )
        else:
            digest_output.parent.mkdir(parents=True, exist_ok=True)
            with tempfile.TemporaryDirectory(
                prefix=f".{digest_output.name}.staging-",
                dir=digest_output.parent,
            ) as digest_temporary:
                staged_digest = Path(digest_temporary) / digest_output.name
                staged_digest.write_text(
                    f"{package.manifest_sha256}\n", encoding="ascii"
                )
                staged_digest.chmod(0o644)
                if _read_limited_file(
                    staged_digest,
                    MANIFEST_DIGEST_MAX_BYTES,
                    "staged manifest digest",
                ).strip() != package.manifest_sha256.encode("ascii"):
                    raise ScriptError("staged manifest digest readback failed")
                _publish_release_directory(
                    staged_destination,
                    release_destination,
                    (staged_digest, digest_output),
                    force=force,
                )
...
def verify_source_tree() -> None:
    ...
    required.update(
        {REPO_ROOT / patch: f"kernel patch {patch}" for patch in _kernel_patch_paths()}
    )
