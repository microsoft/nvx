...
GUEST_AGENT_BUILD_ID = "7097a9dab9c5e4fbc7ef36b93a4ba897996bd052"
SIMPLE_TRANSPORT = "simple"
...
# ELF64 sizes and tags from the System V ABI.
_ELF64_HEADER_BYTES = 64
_ELF64_PROGRAM_HEADER_BYTES = 56
_ELF64_DYNAMIC_ENTRY_BYTES = 16
_ELF_EXECUTABLE_TYPES = (2, 3)  # ET_EXEC and ET_DYN (static PIE).
...
# SVR4 newc: magic, thirteen eight-digit hexadecimal fields, four-byte alignment.
_NEWC_MAGIC = b"070701"
_NEWC_FIELD_BYTES = 8
_NEWC_FIELDS_BYTES = 13 * _NEWC_FIELD_BYTES
_NEWC_HEADER_BYTES = len(_NEWC_MAGIC) + _NEWC_FIELDS_BYTES
_NEWC_ALIGNMENT = 4
...
def validate_shared_status_kernel_config(path: Path) -> None:
    configured = set(path.read_text(encoding="utf-8").splitlines())
    missing = [
        setting
...
def validate_static_x86_64_elf(path: Path) -> None:
    ...
    if len(data) < _ELF64_HEADER_BYTES or data[:6] != b"\x7fELF\x02\x01":
        raise ScriptError("NVX guest agent must be a little-endian ELF64 binary")
    # Offsets below are the ELF64_Ehdr and ELF64_Phdr byte layouts.
    if struct.unpack_from("<H", data, 16)[0] not in _ELF_EXECUTABLE_TYPES:
        raise ScriptError("NVX guest agent must be an ELF executable")
    if struct.unpack_from("<H", data, 18)[0] != _ELF_MACHINE_X86_64:
        raise ScriptError("NVX guest agent must target x86-64")
    program_offset = struct.unpack_from("<Q", data, 32)[0]
    program_entry_size = struct.unpack_from("<H", data, 54)[0]
    program_count = struct.unpack_from("<H", data, 56)[0]
    if program_entry_size != _ELF64_PROGRAM_HEADER_BYTES or program_count == 0:
    ...
    for index in range(program_count):
        ...
        has_load_segment |= segment_type == _ELF_PT_LOAD
        if (
            segment_type == _ELF_PT_LOAD
            and flags & _ELF_PF_EXECUTE
            and virtual_address <= entry_point < virtual_address + memory_size
        ):
            entry_is_executable = True
        if segment_type == _ELF_PT_INTERP:
            raise ScriptError("NVX guest agent must be statically linked")
        if segment_type == _ELF_PT_DYNAMIC:
            if file_size % _ELF64_DYNAMIC_ENTRY_BYTES:
            ...
            terminated = False
            while dynamic_offset + _ELF64_DYNAMIC_ENTRY_BYTES <= dynamic_end:
                dynamic_tag = struct.unpack_from("<q", data, dynamic_offset)[0]
                if dynamic_tag == _ELF_DT_NULL:
                    terminated = True
                    break
                if dynamic_tag == _ELF_DT_NEEDED:
                    raise ScriptError(
                        "NVX guest agent must not have dynamic dependencies"
                    )
                dynamic_offset += _ELF64_DYNAMIC_ENTRY_BYTES
            if not terminated:
                raise ScriptError("NVX guest agent has an invalid dynamic section")
...
def prepare_kernel_source(version: str = DEFAULT_KERNEL_VERSION) -> tuple[Path, str]:
    ...
    fingerprint = kernel_source_fingerprint()
...
@contextmanager
def _prepare_alpine_root(config: AlpineBuildConfig) -> Generator[Path, None, None]:
    ...
    with tempfile.TemporaryDirectory(prefix="root-", dir=config.work) as temporary:
        root = Path(temporary)
        require_tool("tar")
        run_checked(
            [
...
@contextmanager
def _prepare_agent_root(work: Path, agent_source: Path) -> Generator[Path, None, None]:
    with tempfile.TemporaryDirectory(prefix="agent-root-", dir=work) as temporary:
        root = Path(temporary)
        root.chmod(0o755)
        sbin = root / "sbin"
...
@contextmanager
def _require_metadata_preserving_work_directory(
    work: Path,
    ...
    with tempfile.TemporaryDirectory(
        prefix=".nvx-metadata-probe-", dir=work
    ) as temporary:
        probe = Path(temporary)
        # Distinct owner/other permissions and the sticky bit expose mode loss.
        try:
            probe.chmod(0o1701)
        ...
        with output.open("wb") as raw:
            with gzip.GzipFile(
                ...
                for inode, path in enumerate(paths, start=1):
                    ...
                    write(name_bytes)
                    write(b"\0" * (-written % _NEWC_ALIGNMENT))
                    write(content)
                    write(b"\0" * (-written % _NEWC_ALIGNMENT))
                ...
                write(trailer_name)
                write(b"\0" * (-written % _NEWC_ALIGNMENT))
                write(b"\0" * (-written % _NEWC_BLOCK_BYTES))
    except OSError:
...
def _write_apk_manifest(
    ...
    if installed.is_file():
        ...
        raise ScriptError(f"simple initramfs root lacks APK metadata: {installed}")
        ...
        json.dumps(
            {
                ...
                "architecture": "x86_64",
                "profile": BROKER_TRANSPORT
                if config.agent_enabled
                else SIMPLE_TRANSPORT,
                "guest_agent": (
                    {
...
def _newc_entries(path: Path) -> tuple[InitramfsEntry, ...]:
    ...
    while offset < len(data):
        if data[offset : offset + len(_NEWC_MAGIC)] != _NEWC_MAGIC:
            raise ScriptError(f"{path} is not a reproducible newc initramfs")
        header = data[offset + len(_NEWC_MAGIC) : offset + _NEWC_HEADER_BYTES]
        if len(header) != _NEWC_FIELDS_BYTES:
            raise ScriptError(f"{path} has a truncated newc header")
        if any(value not in b"0123456789abcdefABCDEF" for value in header):
            raise ScriptError(f"{path} has an invalid newc header")
        fields = [
            int(header[index : index + _NEWC_FIELD_BYTES], 16)
            for index in range(0, _NEWC_FIELDS_BYTES, _NEWC_FIELD_BYTES)
        ]
        (
            inode,
            mode,
        ...
        name_start = offset + _NEWC_HEADER_BYTES
        ...
        content_start = name_end + (-name_end % _NEWC_ALIGNMENT)
        ...
        if name == "TRAILER!!!":
            ...
            next_offset = content_end + (-content_end % _NEWC_ALIGNMENT)
        ...
        offset = content_end + (-content_end % _NEWC_ALIGNMENT)
    ...
    *,
    ...
    critical = (
        ...
        if agent_profile
            ...
            "etc/shadow": (stat.S_IFREG, 0o640, 0, _ALPINE_SHADOW_GID),
...
def verify_simple_initramfs(path: Path) -> None:
    entries = _validated_initramfs_entries(path, agent_profile=False)
    if "sbin/nvx-agent" in entries:
        raise ScriptError(f"{path} simple profile contains /sbin/nvx-agent")
    try:
        init_entry = entries["init"]
    except KeyError as error:
        raise ScriptError(f"{path} simple profile is missing /init") from error
    expected_init = require_file(REPO_ROOT / "alpine" / "init", "simple /init")
    if (
        ...
        raise ScriptError(f"{path} does not contain the exact simple /init")
...
def build_initramfs(config: AlpineBuildConfig) -> None:
    ...
    with ExitStack() as cleanup:
        cleanup.enter_context(_require_metadata_preserving_work_directory(config.work))
        agent_sha256: str | None = None
        if config.agent_enabled:
            agent_source, agent_sha256 = verified_staged_guest_agent()
            root = cleanup.enter_context(_prepare_agent_root(config.work, agent_source))
            helpers: dict[str, dict[str, str]] = {}
            trusted_owners: dict[str, tuple[int, int]] = {}
        else:
            root = cleanup.enter_context(_prepare_alpine_root(config))
        ...
        if agent_sha256 is not None:
            ...
            verify_simple_initramfs(native_output)
