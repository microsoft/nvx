...
class WinMatchProcess(unittest.TestCase):
    ...
    def test_pid_exact_match(self) -> None:
        self.assertTrue(host._win_match_process("microvm.exe (1234)", self.proc_re, 1234))
        self.assertFalse(host._win_match_process("microvm.exe (1234)", self.proc_re, 9999))
    ...
    def test_name_fallback_without_pid(self) -> None:
        self.assertTrue(host._win_match_process("microvm.exe (1234)", self.proc_re, None))
...
class WinFoldDumper(unittest.TestCase):
    def test_kernel_and_user_fragments_both_survive(self) -> None:
        ...
        folded, cswitch = host._win_fold_dumper(iter(lines), re.compile("microvm"), 1234)
...
class Provenance(unittest.TestCase):
    ...
    def test_matching_run_ids_pass(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            guest.with_name(guest.name + ".run").write_text("run=abc\n", encoding="utf-8")
            trace.with_name(trace.name + ".pid").write_text("pid=9\nrun=abc\n", encoding="utf-8")
...
class ProvenanceExpectedRun(unittest.TestCase):
    ...
    def test_expected_run_overrides_replaced_sidecar(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as d:
            guest = Path(d) / "p.folded"
            trace = Path(d) / "p.host.etl"
            guest.with_name(guest.name + ".run").write_text("run=new\n", encoding="utf-8")
            trace.with_name(trace.name + ".pid").write_text("pid=9\nrun=new\n", encoding="utf-8")
