...
from flamegraph_host import _read_run_id, extract_host_stacks
...
def _check_prereqs(mode: str) -> None:
    ...
    for tool in ["rustfilt", "inferno-flamegraph"]:
        if not _find(tool):
            missing.append(tool)
    if mode == "full":
        if sys.platform == "win32" and not _find("xperf"):
...
def main() -> None:
    ...
    parser.add_argument("mode", choices=["guest", "full"], help="guest-only or guest+host")
    parser.add_argument("--guest-folded", type=Path, required=True, help="guest folded input file")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=None,
    ...
    out_dir = args.output_dir.resolve() if args.output_dir else guest_folded.parent / f"{guest_folded.stem}-flamegraph"
    ...
    if not outputs:
        raise SystemExit(f"[error] no flamegraphs produced (empty guest profile: {guest_folded})")
