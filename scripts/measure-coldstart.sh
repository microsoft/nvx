...
MEM="${MEM:-512}"
N="${N:-6}"
CORES="${CORES:-1}"   # vCPUs per cold-boot run (--vcpus)
...
measure() { # $1=marker  $2..=extra args
    ...
    for _ in $(seq 1 "$N"); do
        timeout 30 "$BIN" --vcpus "$CORES" --kernel "$KERNEL" --initrd "$INITRD" --mem "$MEM" \
...
for _ in $(seq 1 "$N"); do
    timeout 30 "$BIN" --vcpus "$CORES" --kernel "$KERNEL" --initrd "$INITRD" --mem 128 \
