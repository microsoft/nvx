#!/usr/bin/env bash
#
# Demonstrates and measures snapshot/restore with the Python hello-world app:
#   1. cold-boot the VM and time reaching "hello world" (kernel boot + Python startup);
#   2. take a VMM snapshot at the warmed point the app requests one;
#   3. restore from the snapshot N times and time reaching "hello world".
#
set -euo pipefail
...
MEM="${MEM:-256}"
N="${N:-8}"
MARKER="HELLO-WORLD-FROM-PYTHON"
...
echo "== cold boot -> '$MARKER' (kernel boot + full Python startup) =="
for _ in $(seq 1 "$N"); do
