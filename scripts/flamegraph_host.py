...
def extract_host_stacks(
    ...
    if sys.platform == "win32":
        return _extract_windows_etl(guest_folded, out_dir, process_regex, diag, expected_run)
    if sys.platform.startswith("linux"):
        return _extract_linux_perf(guest_folded, out_dir, process_regex, diag, expected_run)
...
def _extract_linux_perf(
    ...
    perf_proc = subprocess.Popen(
        ["perf", "script", "-i", str(perf_data), "-F", "comm,pid,tid,time,event,ip,sym,dso"],
    ...
    if collapse_proc.returncode not in (0, None):
        ...
        print(f"[host] inferno-collapse-perf failed (exit {collapse_proc.returncode}){detail}")
        return None
    if not kept:
        detail = f" ({perf_err})" if perf_err else ""
...
_WIN_STACK_RE = re.compile(r"^\s*Stack,\s*(\d+),\s*(\d+),\s*(\d+),\s*([^,]+),\s*(.+?)\s*$")
...
_LOSS_LEADING_RE = re.compile(r"\b(\d+)\s+(?:events?|buffers?)\s+(?:were\s+)?lost\b", re.IGNORECASE)
_LOSS_MID_RE = re.compile(r"\blost\b\s*[:=]?\s*(\d+)\s+(?:events?|buffers?)\b", re.IGNORECASE)
_LOSS_WORD_RE = re.compile(
    r"\b(?:events?|buffers?)\s+(?:were\s+)?lost\b|\blost\s+(?:events?|buffers?)\b", re.IGNORECASE
)
...
def _xperf_lines(etl: Path, diag: dict | None = None, symbol_path: str | None = None):
    ...
    cmd = ["xperf", "-i", str(etl), "-tle", "-tti", "-symbols", "-a", "dumper", "-stacktimeshifting"]
    ...
    if proc.returncode != 0:
        ...
        raise RuntimeError(f"xperf dumper failed with exit code {proc.returncode}{detail}")
...
def _provenance_ok(guest_folded: Path, trace: Path, expected_run: str | None = None) -> bool:
...
def _extract_windows_etl(
    ...
    if not etl.exists():
        ...
        return None
    if not _provenance_ok(guest_folded, etl, expected_run):
        return None
    if shutil.which("xperf") is None:
        print("[host] xperf not found (Windows Performance Toolkit required)")
        return None

    proc_re = re.compile(process_regex, re.IGNORECASE)
    manifest = _read_host_manifest(etl)
    target_pid: int | None = None
    if "pid" in manifest and manifest["pid"].isdigit():
        target_pid = int(manifest["pid"])
        print(f"[host] filtering host trace to recorder PID {target_pid}")
    else:
        print(f"[host] no PID manifest; filtering host trace by image name /{process_regex}/")
    ...
    try:
        folded, cswitch_count = _win_fold_dumper(
            _xperf_lines(etl, xperf_diag, symbol_path), proc_re, target_pid
        )
    except RuntimeError as e:
        print(f"[host] {e}")
        return None
    lost = bool(xperf_diag.get("events_lost"))
    # Publish loss to the caller so `full` mode does not claim success on an incomplete host trace.
    if diag is not None:
        diag["lost"] = lost
    if lost:
        print(
