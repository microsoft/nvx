...
class CliTests(unittest.TestCase):
    ...
    def test_sandbox_command_parses_typed_launch_contract(self):
        ...
        self.assertEqual(args.transport, "simple")
    ...
    def test_sandbox_command_parses_broker_launch_contract(self):
        args = nvx.parse_args(
            [
                "sandbox",
                "--layer",
        ...
        self.assertIsNone(args.layer[0].uuid)
        self.assertEqual(args.transport, "broker-ttrpc")
...
class SandboxTests(unittest.TestCase):
    def test_simple_launch_orders_roles_and_builds_agent_command_line(self):
    ...
    def test_broker_launch_keeps_bootstrap_config_off_command_line(self):
        ...
        self.assertEqual(launch.broker_kernel_command_line("quiet"), "quiet")
        self.assertEqual(
            launch.broker_openvmm_arguments(
    ...
    def test_launch_contract_rejects_duplicates_and_reserved_tokens(self):
        ...
        broker_launch = sandbox.SandboxLaunch(
            layers=(sandbox.SandboxLayer.parse("distro,distro.erofs"),),
            scratch=Path("scratch.ext4"),
        )
        for command_line in (
            "rdinit=/bin/sh",
            ...
            with (
                ...
                broker_launch.broker_kernel_command_line(command_line)
        with self.assertRaisesRegex(common.ScriptError, "1024-byte"):
            broker_launch.broker_kernel_command_line(
                "x" * sandbox.SANDBOX_COMMAND_LINE_MAX_SIZE
            )
    ...
    def test_layer_parser_rejects_invalid_role_and_shape(self):
        ...
        with self.assertRaisesRegex(common.ScriptError, "ROLE,PATH"):
            sandbox.SandboxLayer.parse("distro,layer.erofs,uuid,ignored")
...
class ReleaseTests(unittest.TestCase):
            ...
            for name, (field, value) in contract_cases.items():
                with self.subTest(name=name):
                    ...
                    invalid_archive = root / (
                        f"{field}.zip" if os.name == "nt" else f"{field}.tar.gz"
                    )
                    write_archive(invalid_archive)
                    binary_destination.write_bytes(b"existing-openvmm")
                    existing_kernel = build_dir / "vmlinux"
                    ...
                    self.assertEqual(existing_kernel.read_bytes(), b"existing-kernel")
                    self.assertEqual(
                        existing_provenance.read_bytes(),
                        b"existing-provenance",
                    )
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                ...
                patch.object(
                    release,
                    "artifact_path",
                    return_value=root / "missing-provenance.json",
                ),
                self.assertRaisesRegex(common.ScriptError, "provenance"),
            ):
                release.package_release(
                    version="1.0.0",
                    destination=destination,
    ...
    def test_force_package_preserves_existing_release_on_staged_hash_failure(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            destination = root / "dist" / "1.0.0"
            destination.mkdir(parents=True)
            ...
            provenance_path = root / build.OPENVMM_PROVENANCE_NAME
            provenance_path.write_text(
                json.dumps(
                    {
                        "format": 1,
            ...
            def staged_hash(path: Path) -> str:
                return binary_sha256 if path == binary else "changed-after-copy"
            ...
            with (
                ...
                self.assertRaisesRegex(common.ScriptError, "binary"),
            ):
                release.package_release(
                    version="1.0.0",
                    destination=destination,
    ...
    def test_release_publication_rolls_back_failed_directory_swap(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            with (
                patch.object(
                    Path, "replace", autospec=True, side_effect=fail_staged_replace
                ),
                self.assertRaisesRegex(OSError, "injected"),
            ):
