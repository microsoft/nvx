...
| --- | --- |
| Base machine | ACPI-free Linux direct boot with Intel MP tables, allowlisted chipset/PMIO, fixed-role sandbox blocks, eight reserved virtio-mmio slots, and 1/2/4/8-vCPU SMP on KVM, MSHV, and WHP. Persisted ABI is value 2 and boot layout is value 3. |
...
| Host filesystem | Fixed no-DAX HostFs, live attachment revalidation, and server-side denied subtrees with alias-identity enforcement. Provider-backed immutable filesystem generations remain outside the current profile. |
