...
| Network | Static identity, fixed transport, the portable in-process Consomme endpoint, MXC egress `allow`/`deny`, fixed ingress `deny`, egress rules, and quiesced restore are implemented. Stateful replies remain available; unrestricted ingress is rejected before launch. Capture drains packet ownership instead of serializing arbitrary pending packets or host flow state. |
