...
| Optional virt-fs | One live virtio-fs device on virtio-mmio at `0xD0001000`. |

The always-on device model is intentionally small. Virtio devices are present only when networking
or a host-directory export is requested.
...
| `0xC0000000..0xFFFFFFFF` | MMIO gap, including virtio-net, virtio-fs, and APIC windows. |
...
and virtio devices transfer requests and payloads through queues allocated in ordinary guest RAM.
...
The VMM owns guest-memory mappings, PVH setup, PMIO exits, optional TAP-backed virtio-net, and the
live POSIX passthrough for virtio-fs. Guest RAM restores use a private mapping of `mem.bin`, so
pages are faulted in lazily and writes do not modify the snapshot.
...
master-PIC IRQ5 on WHP and IRQ10 on KVM; virtio-fs uses IRQ6 on both. Windows filesystem metadata,
rename, capacity, and error behavior are translated to the Linux FUSE ABI. WHP restores map
`mem.bin` copy-on-write with Windows file mappings, retaining the same lazy memory semantics.
...
| `src/net.rs`, `src/virtiofs/` | KVM TAP-backed virtio-net and shared live virtio-fs/FUSE device. |
...
| `FUSE_FS`, `VIRTIO_FS`, `VIRTIO_MMIO` | Mount the VMM's live host-directory export. |
