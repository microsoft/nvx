...
The local Python suite runs with `python -m unittest discover -s scripts -v` on
Linux and Windows. Synthetic install, rollback, interprocess locking, control
contract, and pinned-policy coverage does not require a staged guest agent.
Only the real-agent integration test is gated on `build/nvx-agent` being present.
The separate host-connect tests run from
