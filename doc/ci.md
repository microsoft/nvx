...
supported non-network metrics. Shared guest binaries and benchmark results move
between jobs through runner-compatible Actions caches. Pull requests gate
regressions against recent history, and successful pushes to `dev` append their
p50 values under `benchmarks/baselines/performance`. The runners require the
